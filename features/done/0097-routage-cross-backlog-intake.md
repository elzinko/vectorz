...
- **`plan:order`** (livré par 0089, #52) sait ordonner par ton **plan** `PLAN.md`. Mais il
...
pas la vraie tête du plan `0094` (elle est sur l'autre liste).

## Valeur

Le builder lancé « nu » annonce enfin la **vraie prochaine carte selon TON plan**, quelle que
soit la liste — `0094` en tête. La boucle « mon plan = ce que fait l'outil » est refermée.
...
- Une section **« 🧭 Prochaine selon le PLAN (tous backlogs) »** alimentée en réutilisant
  ...
  (product + statut + `ready` par carte). Partie déterministe = testée (patron 0089 :
...
- [ ] La sortie distingue « tirable » vs « tête bloquée (pas encore ready) » — comme 0089,
      mais cross-backlog.
...
- Suite de **0089** (#52, ordre intra-liste) et de **`portfolio.sh`** (lecture cross-backlog
