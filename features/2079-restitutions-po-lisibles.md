...
3. **Là où la PR naît** — le cas « corps de PR », ajouté le 2026-07-26 (cf. Notes).
   ...
   **plan de test**. Aucun endroit du repo ne compose un corps de PR aujourd'hui (`gh pr
   ...
   | un test de contrat (nouveau) | croiser SKILL.md ↔ règle ↔ asset template, sur le modèle de `src/supervision/__tests__/skill-emission-contract.test.ts` | **aucun test n'assure aujourd'hui le contenu d'une SKILL.md** — c'est exactement le trou de 2095 |
...
- [ ] **Non-récidive mécanique** — retirer la consigne de l'étape PR d'`ezk-sprint` **fait
      rougir** un test de contrat sur le texte des skills (leçon 2095 : un oubli est
