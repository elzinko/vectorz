...
# 0164 — vz-product-builder : l'autonomie par le corpus, pas par le silence
...
(fiche 0154) — chaque décision de corpus = un `gate_reached` journalisé dont le rapport
...
contrat** (critère de la fiche 0154).
...
- Origine : demande PO 2026-07-14 (« avancer sans répondre à mes questions » + « autre
  préfixe pour ne pas écraser mes skills »). Compose : fiche 0154 (kit émetteur,
...
- Depuis la création de cette fiche, `main` a extrait **0057 `ezk-challenge`** — la
  ...
  à terme **composer 0057** (via la formalisation `composes`, fiche 0149) au lieu de
