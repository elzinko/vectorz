...
| 022 | control-plane-ontology | **Accepté** (ontologie) | 🟢 vivant — carte d'ontologie | erratum 2026-07-28 : WIP → Accepté (référence, détails dans 026→034) |
...
| 025 | monorepo-codev | Proposé (§1 **supersédé** par 027) | 🟢 vivant | §1 `mega-city/` racine → **`products/*`** (027 exécuté) ; §2→§6 tiennent |
| 026 | agent-executor-seam | Proposé | 🟢 vivant, différable post-démo | ✅ bannière « révisé par ADR-029 » (0035) — bmad-orchestration gardé *jusqu'à E4* |
| 027 | vectorz-umbrella | Proposé | 🔵 **exécuté** (layout `products/`, name vectorz) | re-tampon Accepté post-démo si PO le souhaite ; renommage package.json = fait |
| 028 | lecteur-journal-moniteur | **Accepté** | 🟢 vivant — **mode nominal Moniteur** | erratum 2026-07-28 : Proposé → Accepté (shippé fiche 0031) |
| 029 | émancipation-bmad-archivage | **Accepté** | 🟢 **acte fondateur époque 2** | E4 *(b)* purge artefacts ✅ 2026-07-28 ; E4 *(c)* code pilote cop1 = ouvert (0039) |
...
| 034 | `.mcp.json` = artefact local | **Proposé** | 🟡 le fichier de branchement Claude Code est **gitignoré** (chemins machine + dépôt public + `SUPERVISION_PER_WORKTREE` neutralisé par un fichier partagé) ; `supervision:link` **est** l'étape d'installation ; racine toujours explicite ⇒ normalisation ADR-019 intacte ; invariant anti-falsification (2050) inchangé | révise le §Proposition de la fiche [2094](../../features/2094-emetteur-branche-sur-claude-code.md) ; `supervision:probe` doit viser le `.mcp.json` **local** ; porte de sortie non construite : scope local `~/.claude.json` |

Cluster cible du pivot : **021 + 022 + 028** (+ 029 qui les opérationnalise).

## Corpus époque 1 : « planning-ADRs » de `_bmad-output/` (ADR-001→014)

Artefacts méthode BMAD — **hors** séquence immuable. **Purge tree 2026-07-28** ;
retrouvables au tag `epoch-1-bmad-final`. État connu : ADR-005 (LLM Routing) supersédé
de facto ; ADR-009 (`sprint-status.yaml` source de vérité) **mort par ADR-029** — le
statut natif vit dans le front-matter des fiches.
