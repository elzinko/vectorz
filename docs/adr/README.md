...
| 037 | grain de merge ≠ grain de revue | **Proposé** | 🟡 livraison d'un lot de sprints : sépare le grain de **merge** (touches sur `main`) du grain de **revue** (feature) ; `ezk-product-builder` gagne `--delivery=per-feature\|per-epic\|batched` ; mode agrégé = **1 `rebase-merge`** sur lot cohérent (même `epic:`), **pas** squash → commits par feature préservés ; `ezk-pr-pilot` réutilisé (test groupé + train de merge), pas réimplémenté ; invariant [ezk-sprint:32](../../products/mega-city/skills/ezk-sprint/SKILL.md) intact (défaut per-feature) | **panel** avant Proposé→Accepté ; arbitrages PO : noms du flag, déclencheur (epic auto vs opt-in vs seuil N), plafond features/PR ; flag sur `ezk-product-builder` (checkpoint inter-sprint) ; fiche d'implémentation liée |
