...
4. **Invariant anti-falsification préservé** (ADR-034) : pour les transports d'**émission de méthode** (MCP / CLI / hooks autonomes), la racine vient de la **config** (`SUPERVISION_PROJECT_ROOT`), jamais d'un paramètre — ni `--root` ni « rattacher à un run existant ». **Exception bornée et auditée** : l'**abandon de siège** (`bin/supervision-abandon.ts <projectRoot> <expected_run_id>`, ADR-035 / fiche 0168) accepte délibérément une racine + un run existant pour **clore** un run orphelin depuis le Moniteur — c'est une action de **siège**, pas d'émission de méthode. Hors cette exception : pas de porte au replay/forge ; le Moniteur reste read-only (ADR-028).
