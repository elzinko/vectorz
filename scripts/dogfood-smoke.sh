...
echo "-- lawgiver bind (projet jetable, pas ~/.claude)"
PROFILE=daily
[[ -f "$MC/profiles/daily.yml" ]] || PROFILE=global
pnpm --dir "$MC" exec tsx bin/lawgiver.ts bind "$PROFILE" "$COBAYE"
test -d "$COBAYE/.claude" || { echo "FAIL: .claude absent après bind ($PROFILE)"; exit 1; }
...
# DIRTY sur le worktree en cours de PR = normal ; le smoke vérifie seulement que le portier tourne.

if [[ -f "$MC/profiles/daily.yml" ]]; then
  echo "-- daily profile présent"
else
  echo "-- daily profile absent (OK — fallback bind=$PROFILE ; arrive avec PR époque 2)"
fi
