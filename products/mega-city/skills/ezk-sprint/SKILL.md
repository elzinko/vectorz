...
(comportement actuel, zéro régression). `github: false` coupe les trois (100 % local).
Selon leur état, adapte la boucle :

| Capacité `off` | Ce que tu changes dans la boucle |
|---|---|
| `pr: false` | **Pas de PR** (étape 8). Le livrable de revue devient le **fichier local** (le rendu de la fiche / `REVIEW.md` du pack de revue), écrit dans le repo. Merge **local** à l'étape 10 (`ezk-pr` → `ship-merge.sh --local`), jamais `gh pr create` ni `gh pr merge`, puis **`ezk-backlog ship <id> local (<sha>)`** pour la transition `shipped` (pas de #PR à inventer, cf. contrat `ship`). |
