...
echo "Fiche 0185 — jointure branche RÉELLE ↔ PR ouverte (injection EZK_ARCHIVE_TEST) :"
GATE_PR="$(EZK_ARCHIVE_TEST=1 EZK_ARCHIVE_TEST_PRS=$'feat/reelle\t42\tdeja ouverte' bash "$CHECK" --gate main)"
FULL_PR="$(EZK_ARCHIVE_TEST=1 EZK_ARCHIVE_TEST_PRS=$'feat/reelle\t42\tdeja ouverte' bash "$CHECK" --full main)"
...
ok "fixture laisse une note visible (anti faux CLEAN silencieux)" \
   "echo \"\$GATE_PR\" | grep -q 'FIXTURE EZK_ARCHIVE_TEST=1'"
ok "sans EZK_ARCHIVE_TEST=1 la variable seule est ignoree (anti-backdoor)" \
   "! EZK_ARCHIVE_TEST_PRS=$'feat/reelle\t99\tx' bash \"$CHECK\" --gate main | grep -q 'pr=#99'"
