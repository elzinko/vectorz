...
- WHEN a specific working directory is genuinely required (a bespoke `scripts/*.sh`), anchor it to the repo root instead of assuming it: `cd "$(git rev-parse --show-toplevel)/<path>" && …` or `bash "$(git rev-parse --show-toplevel)/scripts/<name>.sh"`. `git rev-parse --show-toplevel` resolves the **current worktree's** root, so it stays correct across worktrees.
